...
Единая адаптивная витрина сайта и миниаппа оформлена в
`components/MobileStorefront.tsx`: фотобаннер, поиск, текстовые категории, каталог,
...
На ПК от 900 px используется горизонтальная навигация и компактная сетка;
на телефоне — нижняя панель вкладок. Демонстрационные товары старой ПК-версии
не отображаются. Поиск с главной передаёт введённый запрос в каталог.
Старая реализация `LegacyDesktopApp` сохранена в исходниках, но не является
точкой входа. Декоративные иконки и рекламные «умные подборки» убраны;
небольшие служебные значки обозначают только действия и навигацию.
