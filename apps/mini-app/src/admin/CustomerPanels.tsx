...
type AnalyticsResult = { counter: number; connected: boolean; message?: string; updated_at?: string; local: { customers: number; carts: number; subscribers: number }; totals?: Report; sources?: Report; pages?: Report; engaged?: Report; daily?: Report };
const metric = (value?: number) => value == null ? '—' : value.toLocaleString('ru-RU', { maximumFractionDigits: 1 });
const duration = (value?: number) => value == null ? '—' : `${Math.floor(Math.round(value) / 60)} мин ${Math.round(value) % 60} с`;
export function AnalyticsPanel({ request }: { request: Request }) {
  ...
  const visits = data?.totals?.totals[0];
  const longVisits = data?.engaged?.totals[0];
  const longShare = visits == null || longVisits == null ? undefined : visits > 0 ? longVisits / visits * 100 : 0;
  const sampled = [data?.totals, data?.engaged, data?.daily, data?.sources, data?.pages].some(report => report?.sampled);
  return <section><div className="crm-section-head"><div><h1>Статистика</h1><p className="crm-muted">Яндекс Метрика · счётчик 112522333. В статистике только посетители, разрешившие аналитику; блокировщики могут уменьшать показатели.</p></div><button className="crm-button" onClick={() => refresh(n => n+1)}>Обновить</button></div><label>Период<select value={days} onChange={e => setDays(e.target.value)}><option value="7">7 дней</option><option value="30">30 дней</option><option value="90">90 дней</option></select></label>
    ...
    {data && <><h2>Данные магазина за всё время</h2><div className="crm-stat-grid">{[['Аккаунтов', data.local.customers], ['Непустых корзин', data.local.carts], ['Подписчиков', data.local.subscribers]].map(([name, value]) => <div key={name}><span>{name}</span><strong>{value}</strong></div>)}</div>
      {!data.connected ? <p className="crm-notice">{data.message}</p> : <><h2>Посещения за {days} дней</h2><div className="crm-stat-grid">{['Визиты', 'Уникальные посетители', 'Просмотры страниц', 'Отказы, %'].map((label, index) => <div key={label}><span>{label}</span><strong>{metric(data.totals?.totals[index])}</strong></div>)}<div><span>Среднее время визита</span><strong>{duration(data.totals?.totals[4])}</strong></div></div>
        <h2>На сайте больше минуты</h2><div className="crm-stat-grid"><div><span>Визиты дольше 60 секунд</span><strong>{metric(longVisits)}</strong></div><div><span>Посетители с такими визитами</span><strong>{metric(data.engaged?.totals[1])}</strong></div><div><span>Доля от всех визитов, %</span><strong>{metric(longShare)}</strong></div></div>
        <p className="crm-help">Один посетитель может зайти несколько раз — это разные визиты. Длительность рассчитана Метрикой и не означает непрерывное внимание к экрану. Имена и email анонимных гостей здесь не определяются.</p>
        <p className="crm-help">Обновлено: {date(data.updated_at || '')}. Кэш — до 5 минут. {sampled && 'Данные рассчитаны по выборке; показатели приблизительные.'}</p>
        <section className="crm-customer-card"><h2>Посещаемость по дням</h2><p className="crm-help">Даты в часовом поясе счётчика. Посетителей по дням нельзя складывать для получения уникальных за период.</p>{data.daily?.data.length ? <div className="crm-traffic-table"><table><thead><tr><th scope="col">Дата</th><th scope="col">Визиты</th><th scope="col">Посетители</th><th scope="col">Просмотры</th></tr></thead><tbody>{data.daily.data.map(row => <tr key={row.dimensions[0]?.name}><th scope="row">{row.dimensions[0]?.name}</th>{row.metrics.map((value, index) => <td key={index}>{metric(value)}</td>)}</tr>)}</tbody></table></div> : <p>За выбранный период данных нет.</p>}</section>
