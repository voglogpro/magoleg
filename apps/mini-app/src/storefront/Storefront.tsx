...
import { ArrowLeft, ArrowRight, ArrowLeftRight, Heart, Home, Menu, Search, ShoppingBag, UserRound, X } from 'lucide-react';
...
import { useHashRoute, useStoreData, useStored } from './hooks';
...
function SearchForm({ value = '', onSearch, inputRef, label = 'Найти модель' }: {
  value?: string; onSearch: (value: string) => void; inputRef?: React.RefObject<HTMLInputElement | null>; label?: string;
}) {
  const [query, setQuery] = useState(value);
  useEffect(() => setQuery(value), [value]);
  const submit = (event: FormEvent) => { event.preventDefault(); onSearch(query.trim()); };
  return <form className="sf-search" role="search" onSubmit={submit}>
    <Search size={20} aria-hidden="true" />
    <input ref={inputRef} type="search" aria-label={label} placeholder="Поиск по каталогу" value={query} maxLength={120} onChange={event => setQuery(event.target.value)} />
...
export function Storefront() {
  ...
  const activeFilterCount = [filters.category !== 'all', filters.license !== 'all', filters.stock !== 'all', Boolean(filters.min || filters.max), Boolean(filters.query)].filter(Boolean).length;
  ...
  useEffect(() => {
    if (path === 'catalog' && new URLSearchParams(search).has('focus')) searchRef.current?.focus();
  }, [path, search]);
  ...
  return <div className="sf-store">
    ...
    <header className="sf-header">
      <div className="sf-header__inner">
        ...
        <div className="sf-header-actions">
          <a className="sf-icon-button" href="#catalog?focus=search" aria-label="Поиск товаров"><Search size={22} /></a>
    ...
    <main id="sf-content" className={`sf-main sf-page-${isProduct ? 'product' : path}`} tabIndex={-1} ref={contentRef}>
      ...
      {path === 'home' && <>
        ...
        <div className="sf-home-content">
          <section className="sf-home-selection" aria-labelledby="sf-choose-title">
            ...
            <SearchForm onSearch={query => navigate(catalogHref({ query }))} />
            <div className="sf-home-filters">
              <div><h3>Тип транспорта</h3><div className="sf-choice-grid">{Object.entries(categoryLabels).map(([category, label]) => <a href={catalogHref({ category: category as Filters['category'] })} key={category}>{label}</a>)}</div></div>
              <div><h3>Водительские права</h3><div className="sf-choice-grid"><a href={catalogHref({ license: 'required' })}>С правами</a><a href={catalogHref({ license: 'not-required' })}>Без прав</a></div></div>
            </div>
          </section>
      ...
      {path === 'catalog' && <div className="sf-catalog">
        <SearchForm value={filters.query} onSearch={query => updateFilters({ query })} inputRef={searchRef} />
        <div className="sf-category-tabs" role="group" aria-label="Тип транспорта">{([['all', 'Все модели'], ...Object.entries(categoryLabels)] as string[][]).map(([category, label]) => <button key={category} aria-pressed={filters.category === category} onClick={() => updateFilters({ category: category as Filters['category'] })}>{label}</button>)}</div>
        <div className="sf-catalog-toolbar"><button className="sf-filter-toggle" aria-expanded={filtersOpen} aria-controls="sf-filter-panel" onClick={() => setFiltersOpen(value => !value)}>Фильтры{activeFilterCount > 0 ? ` · ${activeFilterCount}` : ''}</button><label className="sf-sort"><span className="sf-sr-only">Порядок товаров</span><select value={filters.sort} onChange={event => updateFilters({ sort: event.target.value as Filters['sort'] })}><option value="featured">Выбор магазина</option><option value="price-asc">Сначала дешевле</option><option value="price-desc">Сначала дороже</option><option value="name">По названию</option></select></label><div className="sf-view-toggle" aria-label="Вид каталога"><button aria-pressed={layout === 'grid'} onClick={() => setLayout('grid')}>Плитка</button><button aria-pressed={layout === 'list'} onClick={() => setLayout('list')}>Список</button></div></div>
        ...
        {activeFilterCount > 0 && <div className="sf-active-filters" aria-label="Выбранные фильтры">
          ...
          {filters.query && <button onClick={() => updateFilters({ query: '' })}>Поиск: {filters.query}<X size={14} aria-label="Убрать поиск" /></button>}
      ...
      {path === 'cart' && <div className="sf-cart-page">{receipt ? <InquiryConfirmation inquiry={receipt} /> : catalogState || (cart.length ? <div className="sf-cart-layout"><section className="sf-cart-items" aria-label="Выбранные товары"><div className="sf-collection-tools sf-cart-tools"><p>Товаров: {cartCount}</p><button className="sf-text-button" onClick={retry}>Обновить наличие</button><button className="sf-text-button" onClick={() => { setCart([]); setNotice('Корзина очищена.'); }}>Очистить корзину</button></div>{cart.map(item => {
        ...
        })}<a className="sf-text-button sf-continue-shopping" href="#catalog">Продолжить покупки <ArrowRight size={17} /></a></section><aside className="sf-cart-summary"><h2>Ваш выбор</h2><dl><div><dt>Товаров</dt><dd>{cartCount}</dd></div><div><dt>{cartSummary.unknownPrices ? 'Известная стоимость' : 'Стоимость товаров'}</dt><dd>{money(cartSummary.knownTotal)}</dd></div><div><dt>Доставка</dt><dd>Уточняется</dd></div></dl>{cartSummary.unknownPrices > 0 && <p className="sf-muted">Для {cartSummary.unknownPrices} шт. цена будет уточнена. Это не полная сумма заявки.</p>}<p className="sf-muted">Ничего не списываем. Итоговую стоимость и наличие подтверждает магазин.</p><InquiryForm settings={settings} items={cart} blocked={cartSummary.unavailable > 0} onSuccess={inquiry => { setReceipt(inquiry); setCart([]); window.scrollTo({ top: 0, behavior: 'instant' }); }} /></aside></div> : <Empty title="В корзине пока пусто">Добавьте понравившуюся модель, чтобы уточнить её наличие и условия получения у магазина.</Empty>)}</div>}

      {Object.hasOwn(infoTitles, path) && <Information key={path} topic={path} settings={settings} />}
      {path === 'profile' && <div className="sf-profile"><p className="sf-lead">Выбирайте без регистрации</p><p>Избранное и корзина сохраняются в этом браузере. Для обращения в магазин не нужен аккаунт.</p><nav className="sf-account-links"><a href="#favorites">Избранное <span>{favorites.length}</span></a><a href="#compare">Сравнение <span>{compare.length}</span></a><a href="#cart">Корзина <span>{cartCount}</span></a><a href="#contact">Связаться с магазином <ArrowRight size={17} /></a></nav><div className="sf-owner-access"><h2>Управление магазином</h2><p>Закрытый раздел для владельца и сотрудников. Откроется в отдельной вкладке браузера.</p><a className="sf-button sf-button--secondary" href="/admin" target="_blank" rel="noopener noreferrer" title="Откроется в отдельной вкладке">Вход для владельца</a></div></div>}
