...
for (const width of [320, 390, 768, 900, 1440]) {
  ...
  await page.waitForURL(/tag=courier/);
  check(await countIs('.sf-product-card', 1), `${width}: a named theme filters the catalogue`);
  ...
  check(!await page.locator('.sf-login-form [name=consent]').isChecked(), `${width}: registration consent is separate and unchecked`);
